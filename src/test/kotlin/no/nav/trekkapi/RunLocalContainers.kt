...
const val POSTGRES_TEST_USER = "ekstern-trekk-api-db-admin"
...
fun main() {
    ...
    if (START_POSTGRES) {
        ...
        File(localTmp(POSTGRES_JDBCURL_STORAGE)).writeText(dbContainer.jdbcUrl)
    }
    ...
    if (START_KAFKA) {
        ...
        val config = config()
        KafkaTestContainer.createTopic(config().kafkaResponseQueue.topic)
    ...
    if (START_POSTGRES) println("=== DB URL: ${dbContainer?.jdbcUrl}")
