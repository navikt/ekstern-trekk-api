...
fun Route.innmeldingRoutes(
    ...
    post("/v1/innrapportering") {
        ...
        try {
            ...
            call.response.header(HttpHeaders.Location, "${call.request.uri}/$id")
            call.respondText("Accepted, ID=$id", contentType = ContentType.Text.Plain, status = HttpStatusCode.OK)
        } catch (e: Exception) {
